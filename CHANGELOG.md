...
- **Dedicated P&L page** (`/pnl`) — new dashboard page with five sections: summary cards (total balance, unrealized/realized P&L, fees, max drawdown), a multi-view equity curve (Balance / P&L Breakdown / Drawdown tabs via Recharts), open positions table, aggregate trade statistics bar (win rate, profit factor, avg hold time), and a sortable closed-trades journal
- **Closed trades via FIFO lot-matching** — `GET /api/portfolio/trades` reconstructs completed round-trip trades from filled `OrderRecord`s using a FIFO lot queue; supports partial fills and per-lot fee allocation; optional `?symbol=` filter; adapted from the `SimulationRunner` algorithm
- **Trade statistics endpoint** — `GET /api/portfolio/trade-stats` returns aggregate metrics (total trades, win rate, avg P&L, total P&L, profit factor, avg hold hours) computed from FIFO-matched trades; optional `?symbol=` filter
- **Enriched portfolio response** — `GET /api/portfolio/` now includes `total_balance_quote` (cash + mark-to-market value of open positions) and `unrealized_pnl` (sum of position-level unrealized gains/losses)
- **Symbol-scoped P&L bar on Markets page** — compact inline bar below the symbol selector showing unrealized P&L (from current position), realized P&L, and total fees for the selected symbol; data sourced from the portfolio and closed-trades endpoints
- **P&L frontend components** — `PnLSummaryCards`, `RichEquityCurve`, `ClosedTradesTable`, `TradeStatsBar`; all components support dark mode and handle loading/empty/error states
- **`make stop` command** — `uv run python scripts/stop_all.py` forcibly kills lingering Python and Node processes on ports 8000/3000 and removes the Next.js dev lock file; cross-platform (Windows and Unix)
- **GIF capture script** (`scripts/capture_demo/`) — Playwright-based automation that captures dashboard screenshots across pages and stitches them into an animated GIF via Pillow; includes a README with prerequisites and usage instructions
- **15-minute chart timeframe** — `chart_extra_timeframes` setting (default `["15m"]`) fetches and stores 15-minute candles alongside the primary timeframe; `MarketIntelligenceService.run_cycle` accepts `extra_timeframes`; dashboard timeframe selector includes "15M"
- **WebSocket push for market data** — backend poller detects new `TickerRecord` rows and emits `market_data_updated` events; frontend `useWebSocket` hook invalidates chart and ticker queries on receipt, replacing aggressive polling with WS-driven updates (30 s fallback poll as safety net)
